	...
	if($Key != null) {
		...
		if(check_user_cookie($Cookie)) {
			...
			if (check_user_permission($Cookie)) {
				...
				$sql = "SELECT * FROM dinghaopu_rent WHERE (dinghaopu_rent_title LIKE '%" . $Key ."%' OR dinghaopu_rent_name LIKE '%" . $Key . "%' OR dinghaopu_rent_zone LIKE '%" . $Key . "%' OR dinghaopu_rent_industry LIKE '%" . $Key . "%' OR dinghaopu_rent_city LIKE '%" . $Key ."%' OR dinghaopu_rent_phone LIKE '%" . $Key . "%') AND dinghaopu_rent_is_down='已完结' AND dinghaopu_rent_is_approval = '已审核'";
				$result = $conn->query($sql);
				if($result->num_rows > 0) {
		...
		if(check_user_cookie($Cookie)) {
			...
			if (check_user_permission($Cookie)) {
				...
				$data['describe'] = "成功";
				$data['result'] = array();
				$sql = "SELECT * FROM dinghaopu_rent WHERE dinghaopu_rent_is_approval='已审核' AND dinghaopu_rent_is_down='已完结'";
				$result = $conn->query($sql);
				if($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
						$temp = array();
