<?php
	...
	function check_user_permission($cookie) {
		...
		if($result->num_rows > 0) {
			...
			if($row['dinghaopu_user_permission'] == 100) {
				return true;
			}
			else {
				return false;
	...
	if(check_user_cookie($Cookie)) {
		...
		if (check_user_permission($Cookie)) {
			...
			$data['result'] = array();
			$sql = "SELECT * FROM dinghaopu_user";
			$result = $conn->query($sql);
			if($result->num_rows > 0) {
				while($row = $result->fetch_assoc()) {
					$temp = array();						
					$temp['user_id'] = $row['dinghaopu_user_id'];
					$temp['user_name'] = $row['dinghaopu_user_name'];
					$temp['user_permission'] = $row['dinghaopu_user_permission'];
