	...
	if($Key != null) {
		...
		if(check_user_cookie($Cookie)) {
			...
			if (check_user_permission($Cookie)) {
				...
				$data['result'] = array();
				//从标题、名字、商圈、行业搜索，只查询已审核并未转出的店铺
				$sql = "SELECT * FROM dinghaopu_shop WHERE (dinghaopu_shop_title LIKE '%" . $Key ."%' OR dinghaopu_shop_owner_name LIKE '%" . $Key . "%' OR dinghaopu_shop_zone LIKE '%" . $Key . "%' OR dinghaopu_shop_industry LIKE '%" . $Key . "%' OR dinghaopu_shop_city LIKE '%" . $Key ."%' OR dinghaopu_shop_owner_phone LIKE '%" . $Key . "%') AND dinghaopu_shop_is_down='未转出' AND dinghaopu_shop_is_approval='已审核'";
				$result = $conn->query($sql);
				if($result->num_rows > 0) {
		...
		if(check_user_cookie($Cookie)) {
			...
			if (check_user_permission($Cookie)) {
				...
				$data['describe'] = "请求成功，cookie有效，权限有效";
				$data['result'] = array();
				$sql = "SELECT * FROM dinghaopu_shop WHERE dinghaopu_shop_is_approval='已审核' AND dinghaopu_shop_is_down='未转出'";
				$result = $conn->query($sql);
				if($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
						$temp = array();
