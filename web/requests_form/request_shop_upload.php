	...
	if($conn->query($sql)) {
		...
		if ($result->num_rows > 0) {
			...
			for ($i = 0; $i < count($_FILES['images']['name']); $i++) {
				...
				$path0 = "http://zay159.xyz" . substr($path,2);
				$sql = "INSERT INTO dinghaopu_image(`dinghaopu_image_url`, `dinghaopu_image_user_id`) VALUES('{$path0}', {$id});";
				
				$flag = move_uploaded_file($_FILES["images"]["tmp_name"][$i], $path);
				
				if (!$conn->query($sql) || !$flag || !in_array($extension, $allowedExts)) {
					echo '<script>alert("操作失败, 存储图片时错误。");window.history.back(-1);</script>';
					$conn->rollback();
					exit(0);
				}
			}
			echo '<script>alert("操作成功，等待管理员审核。");window.location.href="../pages/shop_list.html";</script>';
			$conn->commit();
		}
		else {
			echo '<script>alert("操作失败，获取ID失败。");window.history.back(-1);</script>';
			$conn->rollback();
		}
	}
	else {
		echo '<script>alert("操作失败，插入时失败。");window.history.back(-1);</script>';
		$conn->rollback();
	}
